...
def adjust_dst(datetime):
    """Converts Standard Time to North American DST. Input to this function is
    a structured time object in xST. The function returns a structured time
    object adjusted to a DST value if appropriate and a flag indicating the DST
    adjustment was made. The helper cannot detect DST for a structured time
    object that is encoded as DST."""

    is_dst = _detect_dst(datetime)  # Determine if datetime is DST
